...
function KeySystem.new(Config, Filename, func, keyValidator)
	...
	local KeyDialog = KeyDialogInit.Create(true, "Popup", Config.Window, Config.AxiomUI, Config.AxiomUI.ScreenGui.KeySystem)
	...
	if Config.KeySystem.API then
		...
		for _, i in next, Config.KeySystem.API do
			local serviceDef = Config.AxiomUI.Services[i.Type]
			if serviceDef then
				local args = {}
