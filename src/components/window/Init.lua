...
return function(Config)
	...
	if not RunService:IsStudio() and Window.Folder and writefile then
		if not isfolder("AxiomUI/" .. Window.Folder) then
			makefolder("AxiomUI/" .. Window.Folder)
		end
		if not isfolder("AxiomUI/" .. Window.Folder .. "/assets") then
			makefolder("AxiomUI/" .. Window.Folder .. "/assets")
		end
	...
	if typeof(Window.Background) == "string" and BGVideo then
		...
		if string.find(BGVideo, "http") then
			...
			if not isfile(videoPath) then
				...
				if not success then
					warn("[ AxiomUI.Window.Background ] Failed to download video: " .. tostring(result))
				end
			end
			...
			if not success then
				warn("[ AxiomUI.Window.Background ] Failed to load custom asset: " .. tostring(customAsset))
			end
			warn("[ AxiomUI.Window.Background ] VideoFrame may not work with custom video")
			BGVideo = customAsset
		end
	...
	Window.UIElements.Main = New("Frame", {
		...
		Config.AxiomUI.UIScaleObj,
		Window.AcrylicPaint and Window.AcrylicPaint.Frame or nil,
		Blur,
		Creator.NewRoundFrame(Window.UICorner, "Squircle", {
			ImageTransparency = 1, --  Window.Transparent and Config.AxiomUI.TransparencyValue or 0,
			Size = UDim2.new(1, 0, 1, 0),
			AnchorPoint = Vector2.new(0.5, 0.5),
			Position = UDim2.new(0.5, 0, 0.5, 0),
			Name = "Background",
	...
	Creator.AddSignal(Window.UIElements.Main.Main.Topbar.Left:GetPropertyChangedSignal("AbsoluteSize"), function()
		...
		local RightWidth = Window.UIElements.Main.Main.Topbar.Right.UIListLayout.AbsoluteContentSize.X
			/ Config.AxiomUI.UIScale

		LeftWidth = Window.UIElements.Main.Main.Topbar.Left.AbsoluteSize.X / Config.AxiomUI.UIScale
		...
		Window.UIElements.Main.Main.Topbar.Center.Position =
			UDim2.new(0, LeftWidth + (Window.UIPadding / Config.AxiomUI.UIScale), 0.5, 0)
		Window.UIElements.Main.Main.Topbar.Center.Size = UDim2.new(
			1,
			-LeftWidth
				- (Window.UIPadding / Config.AxiomUI.UIScale)
	...
	if Window.Topbar.ButtonsType ~= "Default" then
		Creator.AddSignal(Window.UIElements.Main.Main.Topbar.Right:GetPropertyChangedSignal("AbsoluteSize"), function()
			Window.UIElements.Main.Main.Topbar.Left.Position = UDim2.new(
				0,
				(Window.UIElements.Main.Main.Topbar.Right.AbsoluteSize.X / Config.AxiomUI.UIScale) + Window.UIPadding - 4,
				0,
				0
			)
		end)
	...
	if not IsVideoBG and Window.Background and typeof(Window.Background) == "table" then
		...
		Window.UIElements.BackgroundGradient = Creator.NewRoundFrame(Window.UICorner, "Squircle", {
			...
			ImageTransparency = Window.Transparent and Config.AxiomUI.TransparencyValue or 0,
		}, {
			BackgroundGradient,
		})
	end
	...
	function Window:SetBackgroundTransparency(v)
		...
		Config.AxiomUI.TransparencyValue = rounded
	...
	local function SetSize(isAnimation)
		Tween(Window.UIElements.Main, 0.45, {
			Size = not Window.IsFullscreen and CurrentSize or UDim2.new(
				0,
				(Config.AxiomUI.ScreenGui.AbsoluteSize.X - 20) / Config.AxiomUI.UIScale,
				0,
				(Config.AxiomUI.ScreenGui.AbsoluteSize.Y - 20 - 52) / Config.AxiomUI.UIScale
			),
		}, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
	...
	Creator.AddSignal(Config.AxiomUI.ScreenGui:GetPropertyChangedSignal("AbsoluteSize"), function()
	...
	if Config.AxiomUI.UseAcrylic then
	...
	function Window:Open()
		...
		task.spawn(function()
			...
			--[[Config.AxiomUI.UIScaleObj.Scale -= 1 - 0.85
			Tween(
				Config.AxiomUI.UIScaleObj,
				0.33,
				{ Scale = Config.AxiomUI.UIScale },
			...
			Config.AxiomUI:ToggleAcrylic(true)
			--end)
		end)
	end
	function Window:Close()
		...
		Config.AxiomUI:ToggleAcrylic(false)
		...
		--[[Tween(
			Config.AxiomUI.UIScaleObj,
			0.28,
			{ Scale = Config.AxiomUI.UIScale - (1 - 0.85) },
			Enum.EasingStyle.Quint,
			Enum.EasingDirection.Out
		):Play()]]
		if BGImage then
		...
		function Close:Destroy()
			task.spawn(function()
				...
				Config.AxiomUI.ScreenGui:Destroy()
				Config.AxiomUI.NotificationGui:Destroy()
				Config.AxiomUI.DropdownGui:Destroy()
				Config.AxiomUI.TooltipGui:Destroy()
	...
	function Window:ToggleTransparency(Value)
		...
		Config.AxiomUI.Transparent = Value

		Window.UIElements.Main.Background.ImageTransparency = Value and Config.AxiomUI.TransparencyValue or 0
		-- Window.UIElements.Main.Background.ImageLabel.ImageTransparency = Value and Config.AxiomUI.TransparencyValue or 0
	...
	function Window:GetUIScale(v)
		return Config.AxiomUI.UIScale
	end

	function Window:SetUIScale(v)
		Config.AxiomUI.UIScale = v
		Tween(Config.AxiomUI.UIScaleObj, 0.2, { Scale = v }, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
		return Window
	end
	...
	local TabModule = TabModuleMain.Init(Window, Config.AxiomUI, Config.AxiomUI.TooltipGui)
	...
	function Window:Tab(TabConfig)
		...
		return TabModule.New(TabConfig, Config.AxiomUI.UIScale)
	end
	...
	function Window:Dialog(DialogConfig)
		...
		local Dialog = DialogModule.Create(false, "Dialog", Window, Config.AxiomUI, Window.UIElements.Main.Main)
		...
		local function CheckButtonsOverflow()
			...
			local totalWidth = ButtonsLayout.AbsoluteContentSize.X / Config.AxiomUI.UIScale
			local parentWidth = ButtonsContent.AbsoluteSize.X / Config.AxiomUI.UIScale
			...
			if totalWidth > parentWidth then
				...
				if availableSpace > 0 then
					...
					for _, button in ipairs(Buttons) do
						local buttonWidth = button.AbsoluteSize.X / Config.AxiomUI.UIScale
	...
	local CurResizeInput = Config.AxiomUI.GenerateGUID()
		...
		if Window.CanResize then
			...
			Creator.AddSignal(input.Changed, function()
				if input.UserInputState == Enum.UserInputState.End then
					if Config.AxiomUI.CurrentInput and Config.AxiomUI.CurrentInput ~= CurResizeInput then
						return
					end

					Config.AxiomUI.CurrentInput = nil
	...
	Creator.AddSignal(ResizeHandle.InputBegan, function(input)
		...
		then
			...
			Config.AxiomUI.CurrentInput = CurResizeInput
